...
export class UsersDbService {
  ...
  postUser(body: User): void {
    this.http.post<User>(this.apiUrl, body).subscribe((usr) => {
      if (usr != null) {
        console.log(usr);
  ...
  editPhoto(photo: string | ArrayBuffer | null, userId: string) {
    ...
    this.http
      .get<CommentPost[]>(this.commentsUrl + `?userId=${userId}`)
      .subscribe((comments: CommentPost[]) =>
        comments.forEach((comment) => commentsId.push(comment.id as string))
      );
    setTimeout(() => {
      commentsId.forEach((commentId) => {
        this.http
          .patch<CommentPost>(this.commentsUrl + `/${commentId}`, {
            photoUrl: `${photo}`,
          })
    ...
    this.http
      .patch<User>(this.apiUrl + `/${userId}`, { photoUrl: `${photo}` })
      .subscribe((user) => {
        if (user != null) {
  ...
  editPass(userId: string, cypherPass: string) {
    this.http
      .patch(this.apiUrl + `/${userId}`, { idToken: `${cypherPass}` })
